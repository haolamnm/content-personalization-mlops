...
- **Atomic updates**: a change that alters facts updates every dependent doc in the same change — CONTEXT-MAP rows, `docs/agents/decision-log.md`, `.notes/00-roadmap.md` checkboxes, module `CONTEXT.md` (once modules exist).
- **Indexes are contracts**: adding/removing/renaming any note, ADR, skill, rule, or clone updates its index — AGENTS.md mappings, docs/README.md, the generated `docs/**/index.json` (regenerate via script), repos registry.
