...
- **uv owns every environment**: `uv venv` + `uv pip`/`uv sync` per service; the system interpreter (3.9.6, see `COMPUTER.md`) is never used; `uv.lock` is committed per project.
