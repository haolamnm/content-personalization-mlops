...
- **Preflight/postflight mandatory**: `memory_pressure | head -20` + `docker stats --no-stream` around any compose lifecycle; record actual RSS figures into `.computers/MACBOOK.md` §5.
...
- **OrbStack memory is dynamic** (`.computers/MACBOOK.md` §3): set an explicit VM cap before heavy groups rather than trusting reclaim mid-run; do not change the cap without a decision-log row.
