...
- **Benches are gates, not decoration**: `criterion` benches exist for every function on the request path; CI-less for now means the numbers land in `COMPUTER.md` §5 by hand ([`verify-before-done`](verify-before-done.md)).
