...
ADR 0003 sized compose profile groups around MACBOOK's constrained memory: one group at a time, no cross-group live integration. A second Linux machine was provisioned as a remote runtime (hardware, OS, and network facts: [`.computers/THINKBOOK.md`](../../.computers/THINKBOOK.md)), later joined to a Tailscale mesh. ADR 0003's alternative 2 ("remote sandbox for heavy phases — revisit if a phase proves impossible locally") is now revisited early: the constraint is not impossibility but iteration speed and cross-group integration.

## Decision

THINKBOOK is the standing remote runtime for this repo's compose groups:

- **Transport**: Tailscale only (`ssh thinkbook`, MagicDNS name; LAN IP is fallback). No ports exposed to the LAN.
...
- **Port policy**: compose host ports are `${VAR:-default}` overridable so both boxes coexist with local services (ThinkBook runs system postgresql on 5432 → uses `POSTGRES_HOST_PORT=15432`); container-network addresses unchanged.
- **Measurement**: RSS actuals land in the running box's fact file §5 — `.computers/THINKBOOK.md` §7 for remote runs.
- **Scope wall**: WORKSTATION (CloudThinker GCP VM) stays out of MLOps entirely ([`.computers/WORKSTATION.md`](../../.computers/WORKSTATION.md)).
...
- Tailscale becomes infrastructure dependency for deployments; headless fallback = home LAN when both devices share it.
