...
12 unit tests (parser incl. null-literal/null-`created_at` poison cases; positional row mapping with micros-precision instant round-trip; schema order/type/partition guards + rowType-vs-schema drift guard). **E2E-proven on THINKBOOK (2026-08-26)**: 9 gateway events → exactly 2 Iceberg snapshots (added-records 5+4), Parquet in day-partition dirs, catalog row registered; steady RSS 493 MiB. Cold-start contract: **lossy development-only by default** — tail-offset source + ephemeral `/tmp` checkpoints means restarts skip downtime gaps; `CHECKPOINTS_DIRECTORY` env accepts a durable shared URI for restore-capable deployments (durable backend story deferred to deployment-shape ADR).
